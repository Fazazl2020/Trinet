...
----------------------------------------
Change: GatedPositionalEncoding uses MULTIPLICATIVE instead of ADDITIVE
Theory: Based on Wiener filter G(ω) = S²/(S²+N²) and IRM approaches
Expected improvement: +0.15-0.25 PESQ

Modified line 157:
- Original: return X + gate * attn * P_freq_scaled
- Modified: return X * (1.0 + gate * attn * P_freq_scaled)
...
class GatedPositionalEncoding(nn.Module):
    """
    ABLATION 1: MULTIPLICATIVE GATING
    Changed from additive (X + ...) to multiplicative (X * (1.0 + ...))
    Based on Wiener filter theory: G(ω) = S²/(S²+N²)
    """
    ...
    def forward(self, X):
        ...
        # ✅ ABLATION 1: Multiplicative instead of additive
        # Original: return X + gate * attn * P_freq_scaled
        # Modified: Multiplicative gating (Wiener/IRM-like)
