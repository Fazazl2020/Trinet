...
1. Multiplicative gating (X * (1 + gain)) instead of additive
2. Direct frequency band weighting on top of PE

Theory: Tests if BOTH changes together provide additive/synergistic benefits
Expected improvement: +0.20-0.35 PESQ (potentially higher than either alone)

Changes:
- GatedPositionalEncoding: Uses multiplicative (from Ablation 1)
- Added: DirectBandWeighting layer AFTER PE (from Ablation 2 concept)
- Result: X_final = (X * (1 + PE_gain)) * band_mask
...
class DirectBandMask(nn.Module):
    """
    Direct frequency band weighting (second stage)
    Applied AFTER multiplicative PE gating
    """
    def __init__(self, F=257, sample_rate=16000):
        super().__init__()
        self.sample_rate = sample_rate
        self.F = F
        ...
        # Separate learnable gains for direct weighting
        self.direct_gains = nn.Parameter(torch.tensor([1.0, 1.0, 1.0]))
    ...
    def forward(self, X):
        ...
        # Normalize gains (centered around 1.0 for multiplicative)
        gains = F.softmax(self.direct_gains, dim=0) * 3.0

        # Create frequency mask
        freq_mask = torch.ones(freq_bins, device=device)
        freq_mask[:low_end] = gains[0]
        freq_mask[low_end:mid_end] = gains[1]
        freq_mask[mid_end:] = gains[2]
...
class CombinedGatingAndWeighting(nn.Module):
    """
    ABLATION 3: COMBINED - Multiplicative PE + Direct Band Weighting

    Stage 1: Multiplicative PE gating (from Ablation 1)
    Stage 2: Direct band weighting (from Ablation 2 concept)
    ...
    def __init__(self, in_channels, F=257, sample_rate=16000):
        ...
        self.direct_band_mask = DirectBandMask(F=F, sample_rate=sample_rate)
    ...
    def forward(self, X):
        ...
        X_final = self.direct_band_mask(X_pe)
...
class FACLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, F=257, sample_rate=16000):
        ...
        self.combined_gating = CombinedGatingAndWeighting(in_channels, F=F, sample_rate=sample_rate)

    def forward(self, X):
        X = self.combined_gating(X)
        return self.conv(X)
...
class TrinetBSRNN(nn.Module):
    """Trinet Network with Combined Ablation"""
